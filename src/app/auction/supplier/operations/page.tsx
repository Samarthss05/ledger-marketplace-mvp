...
export default function SupplierOperationsPage() {
    ...
    return (
        <div className="mx-auto max-w-7xl space-y-6 px-4 py-7 sm:px-6 lg:px-8">
            <div>
                <p className="text-xs font-bold tracking-[0.16em] text-[#6F9277] uppercase">Deliveries</p>
            ...
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
                ...
                <MetricCard icon={Camera} label="Dispatch photos needed" value={orders.filter((order) => order.verificationStatus === "awaiting_supplier_proof").length.toString()} />
                <MetricCard icon={Truck} label="With Ninja Van" value={orders.filter((order) => ["awaiting_courier_pickup", "in_transit", "awaiting_shop_verification"].includes(order.verificationStatus)).length.toString()} />
                <MetricCard icon={CircleDollarSign} label="Completed order value" value={money(orders.filter((order) => order.payoutStatus === "released").reduce((sum, order) => sum + order.totalPrice, 0))} />
            </div>
                ...
                <div className="fixed inset-0 z-[80] flex items-end justify-center bg-[#142016]/55 p-0 backdrop-blur-sm sm:items-center sm:p-4">
                    <div className="max-h-[95vh] w-full max-w-xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-3xl sm:p-7">
                        <div className="flex items-start justify-between gap-4">
                            <div><p className="text-xs font-bold tracking-[0.15em] text-[#6F9277] uppercase">Dispatch photo</p><h2 className="mt-1 text-xl font-bold text-[#2F312F]">{proofOrder.reference}</h2><p className="mt-1 text-xs text-[#7B817B]">The retailer will use this photo to confirm what arrived.</p></div>
                        ...
                        <label className="mt-5 flex min-h-52 cursor-pointer items-center justify-center overflow-hidden rounded-2xl border border-dashed border-[#BFCBBC] bg-[#F8FAF7]">
                            {photo ? (
                                ...
                                <span className="text-center text-[#7B837B]"><Camera size={25} className="mx-auto" /><span className="mt-2 block text-xs font-semibold">Photograph the packed order</span><span className="mt-1 block text-xs">Show cartons, labels, and full quantity clearly</span></span>
                            )}
                        ...
                        <div className="mt-4 grid gap-4 sm:grid-cols-2">
                            ...
                            <div className="rounded-xl bg-[#F3F7F2] p-4"><p className="text-[11px] font-bold text-[#6F9277] uppercase">Expected quantity</p><p className="mt-1 text-lg font-bold text-[#2F312F]">{proofOrder.quantity} units</p></div>
                        </div>
        ...
        <article className="rounded-3xl border border-[#DDE5DC] bg-white p-5">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                <div className="flex min-w-0 items-start gap-3">
                    ...
                    <div><div className="flex flex-wrap items-center gap-2"><h2 className="text-sm font-bold text-[#2F312F]">{order.productName}</h2><OrderStatus status={order.verificationStatus} /></div><p className="mt-1 text-xs text-[#8A918A]">{order.reference} · Retailer {order.retailerAlias} · {order.quantity} units</p><p className="mt-3 text-sm font-bold text-[#2F312F]">{money(order.totalPrice)}</p></div>
                </div>
                <div className="min-w-72 rounded-2xl bg-[#F7F9F5] p-4"><div className="flex items-center justify-between"><span className="inline-flex items-center gap-2 text-xs font-bold text-[#2F312F]"><Truck size={14} className="text-[#4F6F56]" /> Ninja Van</span><span className="text-[11px] text-[#6F9277]">{order.courier.trackingId ?? "Booking pending"}</span></div><p className="mt-2 text-xs text-[#667066]">{order.courier.lastScan}</p></div>
            </div>
            <div className="mt-5 grid gap-4 border-t border-[#E8ECE7] pt-5 lg:grid-cols-[1fr_280px]">
                <div className="space-y-3">{order.events.slice(-4).map((event) => { const copy = timelineEventCopy(event); return <div key={event.id} className="flex items-start gap-3"><span className="mt-1.5 h-2 w-2 rounded-full bg-[#6F9277]" /><div><p className="text-xs font-semibold text-[#414641]">{copy.title}</p><p className="mt-0.5 text-xs text-[#7B817B]">{copy.detail}</p></div></div>; })}</div>
                <div>
                    {order.verificationStatus === "awaiting_supplier_proof" ? <button type="button" onClick={onProof} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#365845] px-4 py-3 text-xs font-bold text-white"><Camera size={14} /> Add dispatch photo</button> : order.dispute ? <div className="rounded-2xl bg-[#FFF5EC] p-4"><p className="flex items-center gap-2 text-xs font-bold text-[#765031]"><AlertTriangle size={14} /> {order.dispute.reference}</p><p className="mt-2 text-xs leading-5 text-[#765F4C]">{order.dispute.automatedAssessment}</p><p className="mt-2 text-[11px] font-bold uppercase text-[#A66A3A]">{disputeStatusLabel(order.dispute.status)}</p></div> : order.verificationStatus === "verified" ? <div className="rounded-2xl bg-[#EDF6EC] p-4"><p className="flex items-center gap-2 text-xs font-bold text-[#3F7048]"><CheckCircle2 size={14} /> Order completed</p><p className="mt-1 text-xs text-[#607460]">The retailer confirmed the delivery.</p></div> : <ProofCard proof={order.supplierProof} />}
                </div>
            </div>
        </article>
    );
...
function ProofCard({ proof }: { proof?: DeliveryProof }) {
    return (
        <div className="rounded-2xl bg-[#F4F6F3] p-4">
            ...
            <p className="mt-2 flex items-center gap-1 text-[11px] text-[#667066]">
...
function OrderStatus({ status }: { status: FulfillmentOrder["verificationStatus"] }) {
    ...
    return <span className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${style}`}>{supplierOrderStatus(status)}</span>;
}
