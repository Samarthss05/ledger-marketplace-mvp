...
export default function SupplierRequestsPage() {
    ...
    const submit = async () => {
        ...
        try {
            ...
            setSuccess(`${result.reference} submitted with a value score of ${result.score}.`);
    ...
    return (
        <div className="mx-auto max-w-7xl space-y-6 px-4 py-7 sm:px-6 lg:px-8">
            <div>
                <p className="text-[10px] font-bold tracking-[0.17em] text-[#6F9277] uppercase">Private RFQ workspace</p>
                <h1 className="mt-1 text-3xl font-bold tracking-[-0.03em] text-[#2F312F]">Requests & quotes</h1>
                <p className="mt-2 text-sm text-[#707670]">Respond to invited retailer aliases with complete, binding offers.</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
                <Metric icon={FileText} label="Invited RFQs" value={requests.length.toString()} />
                <Metric icon={Clock3} label="Awaiting response" value={requests.filter((request) => !myQuote(request) && ["sent", "quoted"].includes(request.status)).length.toString()} />
                <Metric icon={Send} label="Quotes submitted" value={requests.filter((request) => myQuote(request)).length.toString()} />
                <Metric icon={CheckCircle2} label="Quotes awarded" value={requests.filter((request) => request.awardedQuoteId === myQuote(request)?.id).length.toString()} />
            </div>

            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex flex-wrap gap-2">
                    {(["all", "open", "submitted", "awarded"] as Filter[]).map((value) => (
                        <button type="button" key={value} onClick={() => setFilter(value)} className={`rounded-xl px-3 py-2 text-xs font-semibold ${filter === value ? "bg-[#365845] text-white" : "border border-[#DDE5DC] bg-white text-[#667066]"}`}>{value}</button>
                    ))}
                </div>
                            ...
                            <article key={request.id} className="rounded-3xl border border-[#DDE5DC] bg-white p-5">
                                <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                                    <div className="flex min-w-0 items-start gap-3">
                                        ...
                                        <div>
                                            <div className="flex flex-wrap items-center gap-2">
                                                ...
                                                <span className="rounded-full bg-[#F1F5F0] px-2 py-0.5 text-[9px] font-bold text-[#5B705F]">{request.status}</span>
                                                {won ? <span className="rounded-full bg-[#E7F2E6] px-2 py-0.5 text-[9px] font-bold text-[#3F7048]">Awarded to you</span> : null}
                                            </div>
                                            <p className="mt-1 text-[10px] text-[#8A918A]">{request.reference} · Buyer {request.retailerAlias} · needed {new Date(`${request.deliveryDate}T12:00:00`).toLocaleDateString("en-SG", { dateStyle: "medium" })}</p>
                                    ...
                                    <div className="min-w-64 rounded-2xl bg-[#F7F9F5] p-4">
                                        {quote ? (
                                            <>
                                                <p className="text-[9px] font-bold text-[#6F9277] uppercase">Your offer · {quote.reference}</p>
                                                <p className="mt-2 text-xl font-bold text-[#2F312F]">{money(quote.totalPrice)}</p>
                                                <p className="mt-1 text-[10px] text-[#7B817B]">{quote.deliveryDays} days · {quote.paymentTerms} · score {quote.score}</p>
                                            </>
                                        ) : (
                                            <>
                                                <p className="text-[9px] font-bold text-[#6F9277] uppercase">Target basket</p>
                                                ...
                                                <p className="mt-1 text-[10px] text-[#7B817B]">Retailer ceiling for comparison</p>
                                            </>
                                        )}
                ...
                <div className="fixed inset-0 z-[80] flex items-end justify-center bg-[#142016]/55 p-0 backdrop-blur-sm sm:items-center sm:p-4">
                    <div className="w-full max-w-xl rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-3xl sm:p-7">
                        <div className="flex items-start justify-between gap-3">
                            <div><p className="text-[10px] font-bold tracking-[0.15em] text-[#6F9277] uppercase">Binding supplier offer</p><h2 className="mt-1 text-xl font-bold text-[#2F312F]">{quoteRequest.reference}</h2></div>
                            <button type="button" onClick={() => setQuoteRequest(null)} className="text-xs font-semibold text-[#7B817B]">Cancel</button>
                        </div>
                        <div className="mt-5 grid gap-4 sm:grid-cols-2">
                            <label><span className="mb-1.5 block text-xs font-semibold text-[#414641]">Total quote · SGD</span><input type="number" min={0.01} step={0.01} value={totalPrice} onChange={(event) => setTotalPrice(event.target.value)} className="w-full rounded-xl border border-[#D7DFD5] bg-[#FAFBF9] px-4 py-3 text-sm" /></label>
                            <label><span className="mb-1.5 block text-xs font-semibold text-[#414641]">Lead time · days</span><input type="number" min={1} max={120} value={deliveryDays} onChange={(event) => setDeliveryDays(event.target.value)} className="w-full rounded-xl border border-[#D7DFD5] bg-[#FAFBF9] px-4 py-3 text-sm" /></label>
                        ...
                        <div className="mt-4 flex items-start gap-3 rounded-2xl bg-[#F3F7F2] p-4"><ShieldCheck size={17} className="mt-0.5 text-[#4F6F56]" /><p className="text-xs leading-5 text-[#5E685E]">The retailer sees only {organization?.aliasCode}, pricing, delivery, terms, and performance score—not your company name.</p></div>
                        {actionError ? <p role="alert" className="mt-4 rounded-xl bg-[#FFF2EF] px-4 py-3 text-xs text-[#A33A2B]">{actionError}</p> : null}
                        <button type="button" onClick={() => void submit()} disabled={submitting || Number(totalPrice) <= 0 || Number(deliveryDays) <= 0} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-[#365845] px-4 py-3 text-sm font-bold text-white disabled:opacity-40">{submitting ? <LoaderCircle className="animate-spin" size={16} /> : <Send size={16} />} Submit quote</button>
                    </div>
                </div>
            ) : null}
        </div>
