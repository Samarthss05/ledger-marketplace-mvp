    ...
    const submit = async () => {
        ...
        try {
            await verifyShopDelivery({
                ...
                note: note.trim() || (outcome === "accepted" ? "Received in good condition." : "Evidence attached for review."),
                outcome,
            });
            setVerifyOrder(null);
    ...
    return (
        <div className="mx-auto max-w-7xl space-y-6 px-4 py-7 sm:px-6 lg:px-8">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    ...
                    <p className="mt-2 text-sm text-[#707670]">
                        Track Ninja Van delivery and verify received stock with private evidence.
                    </p>
                </div>
            ...
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
                ...
                <Metric label="Needs verification" value={orders.filter((order) => order.verificationStatus === "awaiting_shop_verification").length.toString()} />
                <Metric label="Protected value" value={money(orders.filter((order) => order.payoutStatus !== "released").reduce((sum, order) => sum + order.totalPrice, 0))} />
            </div>
            ...
            {!loading && orders.length === 0 ? (
                <div className="rounded-3xl border border-dashed border-[#C9D4C6] bg-white py-16 text-center">
                    ...
                    <p className="mt-3 text-sm font-semibold text-[#2F312F]">No fulfillment orders yet</p>
                    <p className="mt-1 text-xs text-[#8A918A]">Award a supplier quote to begin delivery tracking.</p>
                ...
                <div className="fixed inset-0 z-[80] flex items-end justify-center bg-[#142016]/55 p-0 backdrop-blur-sm sm:items-center sm:p-4">
                    <div className="max-h-[95vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-3xl sm:p-7">
                        <div className="flex items-start justify-between gap-4">
                            <div>
                                <p className="text-[10px] font-bold tracking-[0.15em] text-[#6F9277] uppercase">Receiving verification</p>
                                <h2 className="mt-1 text-xl font-bold text-[#2F312F]">{verifyOrder.reference}</h2>
                                <p className="mt-1 text-xs text-[#7B817B]">Compare delivered stock with the supplier handoff evidence.</p>
                            </div>
                        ...
                        <div className="mt-5 grid gap-4 sm:grid-cols-2">
                            <ProofCard title="Supplier handoff" proof={verifyOrder.supplierProof} />
                            <div className="rounded-2xl border border-[#DDE5DC] p-4">
                                <p className="text-xs font-bold text-[#2F312F]">Your receiving photo</p>
                                <label className="mt-3 flex min-h-36 cursor-pointer items-center justify-center overflow-hidden rounded-xl border border-dashed border-[#BFCBBC] bg-[#F8FAF7]">
                                    {photo ? (
                                        ...
                                        <img src={photo.previewUrl} alt="Receiving evidence preview" className="h-44 w-full object-cover" />
                                    ) : (
                        ...
                        <div className="mt-5 grid gap-4 sm:grid-cols-2">
                            ...
                            <label>
                                <span className="mb-1.5 block text-xs font-semibold text-[#414641]">Outcome</span>
                                <select value={outcome} onChange={(event) => setOutcome(event.target.value as Outcome)} className="w-full rounded-xl border border-[#D7DFD5] bg-[#FAFBF9] px-4 py-3 text-sm">
                                    ...
                                    <option value="short">Quantity shortage</option>
                        ...
                        <label className="mt-4 block">
                            <span className="mb-1.5 block text-xs font-semibold text-[#414641]">Receiving note</span>
                        ...
                        {outcome !== "accepted" ? (
                            <div className="mt-4 flex items-start gap-3 rounded-2xl bg-[#FFF5EC] p-4">
                                ...
                                <p className="text-xs leading-5 text-[#765031]">Submitting an issue opens a dispute, preserves both photos and courier scans, and keeps payout on hold for human review.</p>
                            </div>
                        ) : null}
                        ...
                        <button type="button" onClick={() => void submit()} disabled={!photo || Number(quantity) <= 0 || submitting || (outcome !== "accepted" && note.trim().length < 2)} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-[#365845] px-4 py-3 text-sm font-bold text-white disabled:opacity-40">
                            ...
                            {outcome === "accepted" ? "Verify and release payout" : "Open evidence review"}
                        </button>
                    </div>
                </div>
            ) : null}
            ...
            <div className="mt-5 grid gap-4 border-t border-[#E8ECE7] pt-5 lg:grid-cols-[1fr_280px]">
                <div className="space-y-3">
                    {order.events.slice(-4).map((event) => (
                        <div key={event.id} className="flex items-start gap-3">
                            ...
                            <div>
                                <p className="text-xs font-semibold text-[#414641]">{event.title}</p>
                                <p className="mt-0.5 text-[10px] text-[#7B817B]">{event.detail}</p>
                            </div>
                        </div>
                    ))}
                </div>
                <div>
                    {order.verificationStatus === "awaiting_shop_verification" ? (
                        <button type="button" onClick={onVerify} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#365845] px-4 py-3 text-xs font-bold text-white"><Camera size={14} /> Verify delivery</button>
                    ) : order.dispute ? (
                        <div className="rounded-2xl bg-[#FFF5EC] p-4">
                            ...
                            <p className="mt-2 text-[9px] font-bold uppercase text-[#A66A3A]">{order.dispute.status.replaceAll("_", " ")}</p>
                        </div>
                    ) : order.verificationStatus === "verified" ? (
                        <div className="rounded-2xl bg-[#EDF6EC] p-4"><p className="flex items-center gap-2 text-xs font-bold text-[#3F7048]"><ShieldCheck size={14} /> Delivery verified</p><p className="mt-1 text-[10px] text-[#607460]">Evidence matched and payout was released.</p></div>
                    ) : (
                        <div className="rounded-2xl bg-[#F4F6F3] p-4"><p className="flex items-center gap-2 text-xs font-bold text-[#5E685E]"><Clock3 size={14} /> Action not required</p><p className="mt-1 text-[10px] text-[#7B817B]">Waiting for supplier proof or a Ninja Van delivery scan.</p></div>
                    )}
                </div>
            </div>
        </article>
    ...
    return (
        <div className="rounded-2xl border border-[#DDE5DC] p-4">
            ...
            {proof?.photoUrl ? (
                ...
                // eslint-disable-next-line @next/next/no-img-element
                <img src={proof.photoUrl} alt={`${title} evidence`} className="mt-3 h-44 w-full rounded-xl object-cover" />
            ) : (
                <div className="mt-3 flex h-44 items-center justify-center rounded-xl bg-[#F4F6F3] text-[#9AA09A]"><ImageIcon size={22} /></div>
            )}
            <div className="mt-3 flex items-center justify-between text-[10px] text-[#667066]"><span>{proof?.quantity ?? "—"} units</span><span className="inline-flex items-center gap-1"><LockKeyhole size={10} /> Private evidence</span></div>
        </div>
    );
}
